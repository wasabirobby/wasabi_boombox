...
AddEventHandler('wasabi_boombox:saveSong', function()
    local input = lib.inputDialog(locale('save_song'), {locale('name'), locale('youtube_link')})
    if input[1] and input[2] then
        TriggerServerEvent('wasabi_boombox:save', input[1], input[2])
        lib.notify({
            title = locale('success_title'),
            description = locale('song_saved'),
            type = 'success'
        })
    else
        lib.notify({
            title = locale('incorrect_title'),
            description = locale('incomplete_information'),
            type = 'error'
        })
    end
end)
...
AddEventHandler('wasabi_boombox:deleteSong', function(data)
	local confirmed = lib.alertDialog({
		header = locale('delete_song'),
		content = locale('sure_about_delete_song'),
		centered = true,
		cancel = true
	})
	if confirmed == 'confirm' then
		...
		lib.notify({
			...
			description = locale('song_deleted'),
			type = 'success'
		})
	else
		lib.notify({
			title = locale('cancelled'),
			description = locale('cancel_previous_action'),
			type = 'error'
		})
	end
end)
...
AddEventHandler('wasabi_boombox:playMenu', function(data)
    ...
    if data.type == 'play' then
        local keyboard = lib.inputDialog(locale('play_music'), {locale('youtube_link'), locale('distance'), locale('volume')})
        if keyboard then
    ...
    elseif data.type == 'volume' then
        local keyboard = lib.inputDialog(locale('change_volume'), {locale('volume')})
        if keyboard then
    ...
    elseif data.type == 'distance' then
        local keyboard = lib.inputDialog(locale('change_distance'), {locale('distance')})
        if keyboard then
